...
- **跨源重复报价自动折叠**。两个聚合源覆盖面重叠约 1400 条商品，同一条报价会在比价页出现两行；更糟的是店主改过名时两边店名对不上（`priceai=[词生量]` / `aihaotan=[王哥源头]`），而「在售家数」用的是 `COUNT(DISTINCT 店名)`，同一家店会被算成两家（实测「Gemini Pro 一年」虚高 17 家）。同一商品链接只保留一条。

  胜出顺序：**采集日期（UTC 天）新的优先 → 同一天内按源优先级（直采 > AI号探 > PriceAI）→ 再按具体时刻 → 最后按 id**。

  - 为什么同一天要用优先级而不是比时刻：两个聚合源在同一轮夜跑里采集，光比 `verified_at` 等于「谁跑得慢谁赢」（aihaotan 翻 120 页要 4 分钟，priceai 只要 75 秒），那是跑得慢的副产品，不是对数据质量的判断。
  - 为什么日期又要压过优先级：万一某个源挂了几天，不该拿它几天前的旧数据盖掉别人今天刚采的。实测模拟 aihaotan 停更 4 天，1385 条全部让位给 priceai。
  - 直采排最前：实时打源站、有真实缺货状态。聚合源开采前已会跳过本机直采的店铺，这里是兜底。
  - **只在「一个源在该链接上只有一行」时才参与**。有些店铺采不到单品链接，整店商品共用入口 URL 兜底（实测某店 13 个完全不同的商品都挂在同一个 url 上），另有店铺一个商品页挂多个规格（`规格1` ¥11.8 / `规格2` ¥14.8）。那些是真商品，按 URL 盲目去重会把它们抹掉。
  - 结果落成 `raw_offers.shadowed` 一列，每轮采集重算一次（约 0.4 秒），前台查询零额外代价。查询时现算需要三层相关子查询，实测单次超过 3 分钟。
  - 落选的那条只是不展示，数据仍在库里；下一轮重算时若它反超（源恢复更新、或对方停更），会自动翻回来。实测全库 1926 个商品组，折叠前后**「最低价」一个都没变**——折叠掉的只是几百条报价里的一条，几乎不会正好是全组最低价。
...
**数据表**：`canonical_products`（标准品）、`sources`（采集店铺）、`raw_offers`（报价，含 `hidden` 差集下架标记与 `shadowed` 跨源重复标记）、`crawl_runs`（采集日志）、`favorites`（商品收藏）、`favorite_stores`（店铺收藏）、`collection_jobs`（任务队列）。
...
- **升级后跑 `npm run db:init`**：建表是幂等的，同时负责补列和数据迁移（`shadowed` 列的首次回填也在这里）。`applySchema` 不在服务启动时执行。
