...
- **跨源重复报价自动折叠**。两个聚合源覆盖面重叠约 1400 条商品，同一条报价会在比价页出现两行；更糟的是店主改过名时两边店名对不上（`priceai=[词生量]` / `aihaotan=[王哥源头]`），而「在售家数」用的是 `COUNT(DISTINCT 店名)`，同一家店会被算成两家（实测「Gemini Pro 一年」虚高 17 家）。同一商品链接只保留 `verified_at` 最新的一条。
  ...
  - 副作用：折叠掉的是**较旧**的那条，所以某些商品的「最低价」会上浮——那是因为旧价已经不作数了，实测拿代理打源站实时接口仲裁，新的那条才是对的。
