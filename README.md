...
- **已实现的采集器**：`shopApi`（链动小铺）、`dujiao` / `dujiaoHtml`（独角数卡）、`kami`（卡网 / 异次元发卡）、`publicProductsApi`、`productsListApi`、`genericHtml`、`browser`、`priceaiApi`。
- **聚合源** `priceaiApi`。priceai.cc 提供公开无鉴权的聚合接口，收录了数百家发卡站的报价快照，其中约九成是链动小铺——正是直采要靠大陆代理才够得着的那批。接入它等于免费拿到那一大片店铺：**直连即可，不消耗按 IP 计费的代理额度**。
