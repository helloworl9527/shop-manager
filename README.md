...
- **收藏店铺链接**（收藏页）：直接粘店铺链接就能收藏，**不采集商品**，只探测一次店铺名。
  可以设分类（自由填写，已有分类会下拉补全），列表按分类分组。后台给采集店铺点 ★ 的店
  也显示在同一个列表里（带 ★ 标记），两边同步。链动小铺的页面是 SPA、`<title>` 为空，
  直连读不到真名，这类链接会自动走一次代理取 `Shop/info` 里的 nickname；仍拿不到就用
  「域名 / token」兜底，可点「改名」自己填。
