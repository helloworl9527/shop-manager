...
- **夜间模式**：顶栏右上角 ☾/☀ 切换，选择记在 localStorage。**没手动选过时跟随系统**
  （系统切换会实时跟着变，选过之后以本人选择为准）。主题在 `index.html` 的内联脚本里
  于首屏渲染前定好，刷新不会闪白屏。配色全部走 CSS 变量，暗色只覆盖变量表。

- **收藏店铺链接**（收藏页）：直接粘店铺链接就能收藏，**不访问站点、不采集商品**。
  店铺名称和链接都由你自己填，名称留空则用链接推导的「域名 / token」兜底（纯本地处理），
  之后随时可以点「改名」修改。可以设分类（自由填写，已有分类会下拉补全），列表按分类分组。
  后台给采集店铺点 ★ 的店也显示在同一个列表里（带 ★ 标记），两边同步；收藏一个已在采集
  的链接会认领它并点亮 ★，不会出现两条。
