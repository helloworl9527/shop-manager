...
单出口 IP 的总请求量降不下来时（几十家店铺挂同一域名），可以把请求摊到多个出口上。
配置 `SHOP_PROXY_FILE` 指向一个代理列表文件，一行一个：

```
gate.example.com:7778:USER_A:secret
gate.example.com:7778:USER_B:secret
http://user:pass@other.example.com:8080
```

行为要点：

- **按店铺分配、店铺之间轮换**。同一家店铺的全部请求（含风控回退后的浏览器采集）走同一个出口 IP。
  这是刻意的：链动小铺一类站点在一次会话里带 `visitorid` / cookie，中途换 IP 比不用代理更可疑。
- **限速键变成「出口 IP + 域名」**。`SHOP_HTTP_HOST_MIN_GAP_MS` 约束的是单个 IP 上的密度，
  所以 50 个出口的总吞吐是单出口的 50 倍，而每个 IP 看到的密度不变。间隔值不必因为加了代理就调小。
- **代理挂了会自动摘掉**。连接错误 / 407 认证失败会把该代理冷却 `SHOP_PROXY_COOLDOWN_MS`（默认 10 分钟），
  当前店铺立即改用下一个出口。区分「代理故障」和「站点返回 520」——后者不会误伤代理。
- **代理不能指向内网**。列表里的私网地址会在加载时丢弃，目标地址仍然走原有的 SSRF 校验。

代理列表含账号密码，`.gitignore` 已排除 `proxies.txt`，不要提交。

#### 快代理私密代理（动态大陆出口）

静态池只能买到共享大陆 IP，链动小铺会返回 `denied by http_ratelimit`（频率限流）。
要稳定放行，需要**独享大陆住宅出口**——快代理「私密代理」按 IP 数量计费，每次提取
返回一个 1–5 分钟有效的临时 IP，期间流量不限，低频采集不触发地域拒绝与频率限流。
...
- **没有多出口轮换**：全局复用当前提取到的那个临时 IP，失效后自动重提下一个。
- 一个临时 IP 的有效期实测约 1 分钟，因此**采集间隔不宜过大**——同一 IP 上尽量
  多采几家再切下一个，才能摊薄按 IP 计费的成本。实测 2 秒间隔下，一个 IP 可采约 20 家店铺。
...
TUN 模式还会**让代理池完全失效**且不报错：进程设置的 HTTP 代理会被 TUN 层吞掉，
