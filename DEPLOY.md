...
新增了 `favorite_stores` 表（收藏店铺链接）。**拉完代码要跑一次 `npm run db:init`**——
建表是幂等的，同时会把已有的 ★ 收藏店铺搬进这张表；不跑的话收藏页会报没有这张表。
`applySchema` 不在服务启动时执行，只由 `db:init` 触发。

### 失效店铺自动停用

失效的店铺每天都会被重复采一遍：健康面板被刷满，真正要处理的问题被淹掉，链动小铺还要
为此烧一个按个数计费的代理 IP。所以采集结束时会判定「这家是不是确定性失效」，是就自动停用。

**只停用不会自己恢复的**，这是这条规则的全部要害：

| 信号 | 判定 | 需要连续几次 |
|---|---|---|
| 站点返回「店铺链接不存在」等业务答复 | `shop_gone` | 1（站点已明确答复，无需等） |
| 接口返回 HTTP 404 / 410 | `endpoint_gone` | 2（`SHOP_RETIRE_GONE_FAILURES`，临时维护也会 404） |
| 采集结果为空 | `empty` | 7（`SHOP_RETIRE_EMPTY_ROUNDS`，日采一次≈一周） |

**403 / 429 / 5xx / 52x / 网络超时一律不算**。它们看着也像「这家没了」，但那是风控、
地域拒绝或临时故障——链动小铺全靠代理才绕开 403，把它算进来会在代理一抖动时把整批店铺停掉，
而停用不会自己恢复，等于静默丢数据。

其它要点：

- 空店**不能一轮就停**。店主清仓、补货间隙都会采到 0 条，停用后不会自己恢复，
  用户往往过很久才发现。停用那一轮记录的是停用说明而非「采集结果为空」，
  连续计数在此断开——**重新启用后能重新拿满一个完整周期**，不会空一轮就又被停掉。
- 停用原因追加到 `notes` 最前面，**保留用户原有备注**。前台显示「已停用」，点「启用」即可恢复。
- 链动小铺的「店铺链接不存在」原本被 `collectShopApi` 吞掉（`code!==1` 直接 continue），
  表现得和空店一模一样。现在会在「所有 token 都被拒且一条没采到」时抛出来，下游才分得清。
